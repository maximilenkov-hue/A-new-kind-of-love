...
// ==========================================
// 💖 НАСТРОЙКИ РОМАНТИЧЕСКОЙ СТРАНИЦЫ
// ==========================================

// Заголовок страницы
const ROMANTIC_TITLE = "Ты моё вдохновение";

// Тексты для карточек с фото (по порядку: карточка 1, 2, 3)
const CARD_TEXTS = [
    "Каждый миг с тобой — драгоценность, которую я храню в сердце.",
    "Спасибо тебе за смех, нежность и бесконечную доброту.",
    "В этом мире есть настоящее волшебство — это ты."
];

// Тексты между карточками (2 блока)
const TEXT_BLOCKS = [
    "Ты делаешь мир вокруг теплее и светлее. Твоя улыбка способна развеять любые тучи.",
    "Даже когда мы не вместе, я чувствую тепло твоей души. Это как тихая музыка, которая всегда играет во мне."
];
...
function attemptLogin() {
    ...
    if (enteredPassword.toLowerCase() === CORRECT_PASSWORD.toLowerCase()) {
        ...
        // Заполняем контент романтической страницы перед показом
        fillRomanticContent();
...
// ==========================================
// 📝 ЗАПОЛНЕНИЕ РОМАНТИЧЕСКОГО КОНТЕНТА
// ==========================================

function fillRomanticContent() {
    // Заполняем заголовок
    const titleElement = document.querySelector('.romantic-title');
    if (titleElement) {
        titleElement.textContent = ROMANTIC_TITLE;
    }
    
    // Заполняем тексты карточек
    const cardTextElements = document.querySelectorAll('.card-text');
    cardTextElements.forEach((el, index) => {
        if (CARD_TEXTS[index]) {
            el.textContent = CARD_TEXTS[index];
        }
    });
    
    // Заполняем текстовые блоки между карточками
    const textBlockElements = document.querySelectorAll('.text-block p');
    textBlockElements.forEach((el, index) => {
        if (TEXT_BLOCKS[index]) {
            el.textContent = TEXT_BLOCKS[index];
        }
    });
}
